...
Main function edit: took main_CK and added the changed we need for the trigger function (use triggering_algorithm_v2.py in this GitHub with it)
and also added switching i2c clock times for 32 Hz frame rate for the camera trigger
...
from triggering_algorithm_v2 import SQTtrigger
...
while True:
    ...
    if not trigger:
        ...
        if trigger:
            ...
            try:
                frame_taker = MLX90640(i2c=0, address=0x33, sda_pin=4, scl_pin=5, freq = 1000000)  #setting clock freq to 1 GHz
                frame_taker.refresh_rate = RefreshRate.REFRESH_32_HZ                 
    ...
    #trigger = False   this was in here but would make servo activate more than once?
        
    if TTT:
        if counter % 3 == 0: 
            print("Sending science packet")
            TTT.science_packet(trigger, condition, pres, alt, frame, frame_count)
