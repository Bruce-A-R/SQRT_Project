...
from _tuppersat_radio import TupperSatRadio as SatRadio2


class Comms:
  def __init__(self, files, address = 0x53, callsign = 'SQRT',uart_bus = 0, tx_pin = 0, rx_pin = 1):
    """
    files (lst) - List of filenames found on the SD card (HK, Data (before trigger), Data (after trigger), Error Log, Trigger Log
    """
      self.radio_settings = {
                       'address' : address,
                        'callsign' : callsign
 ...
 def get_last_entry(self, filepath):
    """
    Reads the last line from a text file on the SD card. This function will now only be used for querying the error log
